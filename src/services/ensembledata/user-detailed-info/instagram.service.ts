...
/**
 * Get multiple Instagram posts details (for batch processing)
 */
export async function getMultipleInstagramPosts(
  inputs: InstagramPostInput[]
): Promise<ProcessedInstagramData[]> {
  console.log(`🔄 Instagram Service: Batch processing ${inputs.length} posts`);
  
  const results: ProcessedInstagramData[] = [];
  
  for (const input of inputs) {
    try {
      const result = await getInstagramPostDetails(input);
      results.push(result);
      
      // Add small delay between requests to avoid rate limiting
      await new Promise(resolve => setTimeout(resolve, 100));
    } catch (error) {
      console.error('💥 Instagram Service: Error in batch processing:', error);
      results.push({
        user: {
          user_ig_id: '',
          full_name: '',
  ...
  console.log(`✅ Instagram Service: Batch processing completed. ${results.filter(r => r.success).length}/${inputs.length} successful`);
  return results;
}